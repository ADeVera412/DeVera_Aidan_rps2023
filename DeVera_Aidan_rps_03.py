...
#For the rest of the images, set up the positions on the canvas of turtle
def show_rock(x,y):
    screen.addshape(rock_image)
    rock_instance.shape(rock_image)
    rock_instance.penup()
...
# Have the computer select a random choice between rock, paper, and scissors
def cpu_select():
...
#Set the choices for the users
show_rock(-300,0)
show_paper(0,0)
show_scissors(300,0)

# function that passes through wn onlick
# This sets up conditions that decides who wins and loses and what results of that
def mouse_pos(x, y):
    print(cpu_select())
    ...
    if collide(x,y,rock_instance,rock_w,rock_h):
        ...
        play_rock()
        if cpu_picked == "rock":
            #Say where the computer choice ends up
            cpu_show_rock(-300,0)
            #Hide unwanted and irrelevant elements
            rock_instance.hideturtle()
            scissors_instance.hideturtle()
            paper_instance.hideturtle()
            #Reset the text in order to print the final result
            text.clear()
            text.penup()
            text.setpos(0,150)
        ...
        if cpu_picked == "paper":
         ...
         text.penup()
         text.setpos(0,150)
         text.write("You lost...", False, "left", ("Arial", 24, "normal"))   
